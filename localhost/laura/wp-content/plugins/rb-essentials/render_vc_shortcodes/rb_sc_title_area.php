...
function rb_vc_shortcode_sc_title_area ( $atts = array(), $content = "" ){
	...
	if( $customize_size ){
		...
		if( !empty($meta_size) ){
			$styles .= "
				#".$id." .single_post_meta a{
					font-size: ".(int)esc_attr($meta_size)."px;
				}
			";
		}
	...
	if( 
		...
		";
			...
			if( $customize_size_landscape ){
				...
				if( !empty($meta_size_landscape) ){
					$styles .= "
						#".$id." .single_post_meta a{
							font-size: ".(int)esc_attr($meta_size_landscape)."px;
						}
					";
				}
	...
	if( 
		...
		";
			...
			if( $customize_size_portrait ){
				...
				if( !empty($meta_size_portrait) ){
					$styles .= "
						#".$id." .single_post_meta a{
							font-size: ".(int)esc_attr($meta_size_portrait)."px;
						}
					";
				}
	...
	if( 
		...
		";
			...
			if( $customize_size_mobile ){
				...
				if( !empty($meta_size_mobile) ){
					$styles .= "
						#".$id." .single_post_meta a{
							font-size: ".(int)esc_attr($meta_size_mobile)."px;
						}
					";
				}
	...
	$out .= "<div id='".$id."' class='custom page_title_container ".$extra_classes."' ".$extra_styles.">";
		...
		if( !empty($interactive_img) ){
			...
			$out .= "<img data-depth='0.80' src='".esc_url($dynamic_image)."' class='page_title_dynamic_image' alt='".get_bloginfo( 'name' )."' />";
		}

		$out .= "<div class='page_title_wrapper'>";

			if( is_singular('post') && strripos($hide_fields, 'cats') === false ){
				$out .= "<div class='single_post_categories'>";
					ob_start();
						the_category(' ');
					$out .= ob_get_clean();
				$out .= "</div>";
			...
			if( is_singular('post') && strripos($hide_fields, 'meta') === false ){
				...
				$out .= "<div class='single_post_meta'>";
					...
					$out .= "<a href='".esc_url($date)."'>".esc_html( get_the_date( get_option( 'date_format' ) ) )."</a>";
					
					//Comments
					$out .= "<span class='coments_count'><a href='#comments'>".$comments."</a></span>";
