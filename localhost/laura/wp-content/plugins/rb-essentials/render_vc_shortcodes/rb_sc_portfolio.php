...
function rb_vc_shortcode_sc_portfolio ( $atts = array(), $content = "" ){
	...
	$defaults = array(
		...
		"meta_color"					=> rb_Hex2RGBA( PRIMARY_COLOR, '0.8' ),
		"divider_color"					=> SECONDARY_COLOR,
		"background_color"				=> rb_Hex2RGBA( SECONDARY_COLOR, '0.96' ),
	);
	...
	if( !empty($meta_color) ){
		$styles .= "
			...
			#".$id." .rb_portfolio_items .rb_portfolio_item p > a:after{
	...
	if( !empty($divider_color) ){
		$styles .= "
			#".$id." .rb_portfolio_items .rb_portfolio_item .text_info .h5 span{
	...
	if( $is_carousel ){
		...
		$carousel_atts .= $layout == 'carousel' ? " data-tablet-portrait=2" : " data-columns=1";
		$carousel_atts .= $infinite_carousel ? " data-infinite=on" : "";
	...
	if( $final_q->have_posts() ):
		ob_start();
			...
			echo "<div id='".$id."' class='".$module_classes."' ".$module_atts.">";
				...
				if( $layout == 'grid_filter' ){
					echo "<ul class='portfolio-filter'>";
						...
						echo "<li class='h4 portfolio-filter-trigger active' data-value='all'>".esc_html_x('All', 'frontend', 'ogo')."</li>";
						foreach( $terms as $term ){
							echo "<li class='h4 portfolio-filter-trigger' data-value='f_".$term."'>".esc_html($term)."</li>";	
						}
						...
						foreach ($temp_terms as $term) {
							...
							foreach ($final_q->posts as $post) {
								...
								if( in_array($term, $post_terms) && $count == 0 ){
									...
									$images[] = get_the_post_thumbnail_url($post->ID, $thumb_size);
						...
						while( $final_q->have_posts() ) : $final_q->the_post();
							...
							echo '<div class="rb_portfolio_item preloaded all '.esc_attr($filter_atts).'" data-masonry-width="'.$masonry_width.'" data-masonry-height="'.$masonry_height.'">';
												...
												if( $layout == 'carousel_wide' ){
													echo '<i class="open_info">';
														echo '<svg x="0px" y="0px" width="28.208px" height="28.333px" viewBox="0 0 28.208 28.333" enable-background="new 0 0 28.208 28.333" xml:space="preserve">
																<line fill="#353535" stroke="#353535" stroke-miterlimit="10" x1="26.873" y1="1.522" x2="1.463" y2="26.67"></line>
																<line fill="#353535" stroke="#353535" stroke-miterlimit="10" x1="27.232" y1="0.64" x2="27.177" y2="10.981"></line>
																<line fill="#353535" stroke="#353535" stroke-miterlimit="10" x1="27.716" y1="1.151" x2="17.374" y2="1.098"></line>
																<line fill="#353535" stroke="#353535" stroke-miterlimit="10" x1="1.074" y1="27.75" x2="1.13" y2="17.409"></line>
																<line fill="#353535" stroke="#353535" stroke-miterlimit="10" x1="0.591" y1="27.239" x2="10.933" y2="27.293"></line>
															</svg>';
													echo '</i>';
													...
													echo '<div class="advanced_info">';
														...
														echo '<div class="advanced_details">';
															echo '<div class="advanced_detail">';
																echo '<p class="label">'.esc_html_x('Date', 'frontend', 'ogo').'</p>';
																echo get_the_date();
															echo '</div>';

															if( !empty($author) ){
																echo '<div class="advanced_detail">';
																	echo '<p class="label">'.esc_html_x('Author', 'frontend', 'ogo').'</p>';
															...
															if( !empty($client) ){
																echo '<div class="advanced_detail">';
																	echo '<p class="label">'.esc_html_x('Client', 'frontend', 'ogo').'</p>';
								...
								if( $hover == 'swipe_right' ){
									echo '<div class="text_info">';
										if( strripos($portfolio_hide_meta, 'title') === false ){
											echo '<a href="'.get_the_permalink().'" class="h5">';
												echo get_the_title();
												echo '<span></span>';
											echo '</a>';
										}
			...
			if( !$is_carousel ) ogo__pagination( $final_q, $total_items_count, $items_pp, $pagination, $pages_quantity );
...
function rb_portfolio_scripts() {
	...
	wp_register_script( 'rb-portfolio', plugin_dir_url( __FILE__ ) . '../assets/js/rb-portfolio.js', array('ogo-theme'), OGO__VERSION, true );
		...
		while( have_posts() ): the_post();
			...
			echo '<div class="rb_portfolio_item all '.esc_attr($filter_atts).'" data-masonry-width="'.$masonry_width.'" data-masonry-height="'.$masonry_height.'">';
				...
				if( !empty( get_the_post_thumbnail(get_the_ID()) ) ){
					echo '<div class="image_wrapper hover_'.esc_attr($hover).'" style="background-image: url('.get_the_post_thumbnail_url(get_the_ID(), 'full').')">';
						echo '<a href="'.get_permalink().'">';
							echo $layout == 'pinterest' ? get_the_post_thumbnail(get_the_ID(), $thumb_size) : '';
						echo '</a>';
						if( $hover != 'swipe_right' ){
							echo '<div class="hidden_info">';
