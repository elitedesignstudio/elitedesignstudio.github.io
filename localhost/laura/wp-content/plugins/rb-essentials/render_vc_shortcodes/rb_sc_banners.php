...
function rb_vc_shortcode_sc_banners ( $atts = array(), $content = "" ){
	$defaults = array(
		...
		"button_title_hover"			=> SECONDARY_COLOR,
	...
	if( $customize_colors ){
		...
		if( !empty($button_title_hover) ){
			$styles .= "
				#".$id." .rb_button:hover{
					color: ".esc_attr($button_title_hover).";
				}
			";
		}
	...
	$out .= $start_tag." id='".$id."' class='".$module_classes."' ".$module_atts.">";
		...
		if( !empty($button_title) ){
			$out .= "<div class='rb_button_wrapper'>";
				$out .= "<a class='rb_button ".esc_attr($button_size)."' href='".esc_url($banner_url)."' ".($new_tab ? 'target="_blank"' : '').">";
