...
function rb_vc_shortcode_sc_progress_bar ( $atts = array(), $content = "" ){
	$defaults = array(
		...
		'percents_color'			=> THIRD_COLOR,
		'gradient_color_1'			=> THIRD_COLOR,
		'gradient_color_2'			=> THIRD_COLOR,
		'bar_bg_color'				=> "#FFF9BB",
		'el_class'					=> '',
 	);
	...
	if( !empty($bar_bg_color) ){
		$styles .= "
			#".$id." .progress_bar{
				background-color: ".esc_attr($bar_bg_color).";
			}
		";	
	}
	...
	$out .= "<div id='".$id."' class='rb_progress_bar_module ".esc_attr($el_class)."'>";
		if( !empty($title) ){
			$out .= "<p class='h6'>".esc_html($title)."</p>";
		}
