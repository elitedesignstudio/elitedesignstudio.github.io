...
defined( 'ABSPATH' ) or die(); ?>
	...
	<div id="site-header" class="site-header">
		<div class="header-content">
				...
				<div class="container <?php if( get_theme_mod('menu_wide') ) echo 'wide_container'; ?>">
					<div class="site_logotype">
						<a href="<?php echo esc_url( home_url('') ) ?>">
							<?php ogo_logo('logotype', 'logo_dimensions') ?>
						</a>
					</div>
					...
					<?php if( 
						get_theme_mod('menu_add_search') || 
						(class_exists('WooCommerce') && get_theme_mod('menu_mini_cart')) || 
						!empty(get_theme_mod('menu_button_title'))
					) : ?>

						<div class="menu-right-info">

							<?php if( get_theme_mod('menu_add_search') ) : ?>
								<div class="search-trigger"></div>
							<?php endif; ?>

							<?php 
								if( class_exists('WooCommerce') && get_theme_mod('menu_mini_cart') ){

									$rb_woocommerce = new Ogo_WooExt();
			...
			<?php if( ( OGO__ACTIVE && !is_front_page() && !is_404() ) || ( !OGO__ACTIVE && !is_404() ) ) : ?>
				<?php
				...
				<?php if( empty(rb_get_metabox('slider_shortcode')) ): ?>
					<div class="page_title_container<?php echo sprintf('%s', $title_extra_classes) ?>" <?php echo sprintf('%s', $title_extra_styles) ?>>
						...
						<?php if( rb_get_metabox('title_interactive_remove') != 'on' && !empty($dynamic_image) ) : ?>
							<img data-depth="0.80" src="<?php echo esc_url($dynamic_image) ?>" class="page_title_dynamic_image" alt="<?php echo get_bloginfo( 'name' ); ?>" />
						<?php endif; ?>

						<div class="page_title_wrapper container">
							<?php
								if( is_singular('post') && strripos($title_hide, 'cats') === false ){
									echo "<div class='single_post_categories'>";
										the_category(' ');
									echo "</div>";
								}
							?>
