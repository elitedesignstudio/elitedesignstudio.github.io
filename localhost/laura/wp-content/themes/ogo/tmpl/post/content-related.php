							...
							<div id="post-<?php the_ID() ?>" <?php post_class( $extra_class ) ?>>
								<div class="post-inner">
									<div class="post-media-wrapper">
										<!-- Featured Media -->
										<?php echo ogo__post_featured( $hide_meta, get_theme_mod('blog_related_cropp') ) ?>

										<!-- Post Date -->
										<?php ogo__post_date( $hide_meta, 'complex' ) ?>
									</div>
									
									<!-- Post Categories -->
									<?php ogo__post_category( $hide_meta ) ?>

									<!-- Post Title -->
									<?php ogo__post_title( $hide_meta ) ?>

									<?php if( strripos($hide_meta, 'author') === false || ( strripos($hide_meta, 'comments') === false && get_comments_number() != 0 ) ) : ?>
										<div class="post-meta-wrapper">
											<!-- Post Author -->
											<?php ogo__post_author( $hide_meta ) ?>

											<!-- Post Comments -->
											<?php ogo__post_comments( $hide_meta ) ?>
										</div>
									<?php endif; ?>

									<?php 
										$content = ogo__the_content( get_theme_mod('blog_related_text_length'), get_theme_mod('blog_read_more'), $hide_meta );
										if( !empty($content) ) : 
									?>
										<!-- Post Content -->
