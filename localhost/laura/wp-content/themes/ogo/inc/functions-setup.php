...
add_action( 'after_setup_theme', 'ogo__navigation' );

// Setup theme supports
add_action( 'after_setup_theme', 'ogo__supports' );
...
// for the theme
add_action( 'init', 'ogo__register_assets', 5 );

// We need enqueue the scripts and styles before showing
// the content
add_action( 'wp_enqueue_scripts', 'ogo__enqueue_assets', 5 );
add_action( 'wp_enqueue_scripts', 'ogo__enqueue_assets', 5 );

// Enqueue fonts in footer to avoid render-block
add_action( 'get_footer', 'ogo__enqueue_fonts' );
add_action( 'get_footer', 'ogo__typography_styles' );

// Setup sidebars & widgets
add_action( 'widgets_init', 'ogo__widgets_init' );

// Adding SVG support in the media library
add_filter( 'upload_mimes', 'ogo__upload_mimes' );
 ...
 * @since   1.0.0
 */
function ogo__navigation() {
	register_nav_menus( array(
		'primary'   => esc_html__( 'Primary Menu', 'ogo' ),
		'sliding'   => esc_html__( 'Sliding Menu', 'ogo' ),
		'top'       => esc_html__( 'Top Menu', 'ogo' )
	) );
}
 ...
 * @return  void
 */
function ogo__register_assets() {
	// Theme's styles
	wp_register_style( 'ogo-theme', get_template_directory_uri() . '/assets/css/main.css', array(), OGO__VERSION, 'all' );

	// Theme's icons
	wp_register_style( 'rbicons', get_template_directory_uri() . '/assets/fonts/rbicons/style.css', array(), OGO__VERSION, 'all' );
	...
	wp_register_script( 'ogo-theme', get_template_directory_uri() . '/assets/js/theme.js', array( 'rb-magnific-popup', 'rb-waypoints', 'rb-counterup', 'rb-particles', 'rb-sticky', 'rb-tilt', 'rb-isotope' ), OGO__VERSION, true );
}

function ogo__enqueue_assets() {
	// The dynamic styles
	...
	wp_enqueue_style( 'ogo-theme' );

	// Enqueue the inline stylesheet
	wp_add_inline_style( 'ogo-theme', ogo__styles() );
	wp_add_inline_style( 'ogo-theme', ogo__scheme_styles() );

	// Enqueue the main script
	wp_enqueue_script( 'rb-slick-slider' );
	wp_enqueue_script( 'ogo-theme' );
...
function ogo__enqueue_fonts() {
	// Enqueue flaticons
    $rbfi = get_option('rbfi');
    if( !empty($rbfi) && isset($rbfi['css']) ){
        wp_enqueue_style( 'rbfi-css', $rbfi['css'], array(), OGO__VERSION, 'all' );
    } else {
        wp_enqueue_style( 'flaticons', get_template_directory_uri() . '/assets/fonts/flaticons/style.css', array(), OGO__VERSION, 'all' );
    };
 ...
 * @return  array
 */
function ogo__upload_mimes( $mimes ){
...
function rb_check_for_update( $transient ){
	...
	$theme_pc = trim(get_option('envato_purchase_code_ogo'));
	if (empty($theme_pc)) {
		add_action( 'admin_notices', 'rb_an_purchase_code' );
	}

	$result = wp_remote_get('http://up.rainbow-themes.net/products-updater.php?pc=' . $theme_pc . '&tname=' . 'ogo');
...
function rb_an_purchase_code() {
	...
	echo "<div class='update-nag'>" . $rb_theme->get('Name') . esc_html__(' theme notice: Please insert your Item Purchase Code in Theme Options to get the latest theme updates!', 'ogo') .'</div>';
}
# /Theme and plugin updates