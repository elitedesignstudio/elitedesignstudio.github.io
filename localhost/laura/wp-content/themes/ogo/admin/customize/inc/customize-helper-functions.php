...
function ogo_sanitize_dropdown_pages( $page_id, $setting ) {
 ...
 * @return  mixed
 */ 
function ogo_read_options( $wp_customize, $extensions ){
    $ajax_data = array();
    $count = 0;
                            ...
                            foreach( $section_value as $k => $v ){
                                ...
                                foreach ( $v as $settings_key => $settings_value ){
                                    ...
                                    switch( $settings_key ){
                                        ...
                                        case 'choices':
                                            if( $control_atts['type'] == 'multiple_input' && is_multi_array($settings_value) ){
 ...
 * @return  mixed
 */ 
function ogo_get_defaults( $extensions ){
    $defaults = array();
                        ...
                        if( $section_key == 'layout' ){
                            foreach ($section_value as $k => $v) {
                                foreach ($v as $setting_key => $setting_value) {
                                    ...
                                    if( $v['type'] == 'multiple_input' && is_array($setting_value) && is_multi_array($setting_value) ){
...
function ogo_get_google_fonts(){
    ...
    if( get_option('rb_g_fonts') ){
        ...
        $file = OGO__PATH . 'assets/fonts/gf.json';
 ...
 * @return  mixed
 */	
function ogo_typography_control( $location, $def_family, $def_weight, $def_subset, $def_color, $def_size, $def_height ){
    $font_choices = array();
    ...
    if( !$def_c ){
        ...
        $def_c = array(
            $loc.'_font_color' => array(
                'type'      => 'alpha-color',
                'label'     => esc_html_x(ucfirst($loc).' font Color', 'customizer', 'ogo'),
                'default'   => $def_c,
    ...
    if( $def_s && $def_h ){
        $f_sizes = array(
            $loc.'_font_size' => array(
                'type'      => 'text',
                'default'   => $def_s,
                'label'     => esc_html_x(ucfirst($loc).' font size', 'customizer', 'ogo')
            ),
            $loc.'_font_height' => array(
                'type'      => 'text',
                'default'   => $def_h,
                'label'     => esc_html_x(ucfirst($loc).' line height', 'customizer', 'ogo'),
                'separator' => 'line'
            )
        );
    } else {
    ...
    $out = array_merge(
        array(
            $loc.'_font_family' => array(
                'type'      => 'select',
                'label'     => esc_html_x(ucfirst($loc).' font family', 'customizer', 'ogo'),
                'default'   => $def_f,
                'choices'   => $font_choices
            ),
            $loc.'_font_weight' => array(
                'type'          => 'select',
                'label'         => esc_html_x(ucfirst($loc).' font weight', 'customizer', 'ogo'),
                'default'       => $def_w,
                'choices'       => $font_weight,
                'input_attrs'   => array(
                    'multiple' => true
            ...
            $loc.'_font_subset' => array(
                'type'          => 'select',
                'label'         => esc_html_x(ucfirst($loc).' font subsets', 'customizer', 'ogo'),
                'default'       => $def_ss,
