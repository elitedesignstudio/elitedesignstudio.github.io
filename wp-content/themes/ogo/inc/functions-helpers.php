...
/**
 * Helper function that removs extra p tags
 * 
 * @return  string
 */
...
function rb_get_page_title(){
	...
	if( is_404() ){
		$page_title = esc_html_x('404', 'frontend', 'ogo');
	} else if( is_front_page() ){
		$page_title = esc_html_x('Home', 'frontend', 'ogo');
	} else if( is_search() ){
		$page_title = esc_html_x('Search', 'frontend', 'ogo');
	} else if( is_category() ){
		$cat = get_category( get_query_var( 'cat' ) );
		$cat_name = isset( $cat->name ) ? $cat->name : '';
		$page_title = sprintf( esc_html_x('Category: %s', 'frontend', 'ogo'), $cat_name );
	} else if( is_tag() ){
		$page_title = sprintf( esc_html_x('Tag: %s', 'frontend', 'ogo'), single_tag_title( '', false ) );
	} else if( is_day() ){
		...
		$page_title = !empty(get_theme_mod('woo_slug')) ? esc_html(get_theme_mod('woo_slug')) : esc_html_x('Shop', 'frontend', 'ogo');;  
	} else if( get_post_type() == 'rb_staff' ){
		if( is_singular() ){
			$page_title = !empty(get_theme_mod('rb_staff_single_slug')) ? esc_html(get_theme_mod('rb_staff_single_slug')) : esc_html_x('Our Team Single', 'frontend', 'ogo');
		} else {
			$page_title = !empty(get_theme_mod('rb_staff_slug')) ? esc_html(get_theme_mod('rb_staff_slug')) : esc_html_x('Our Team', 'frontend', 'ogo');
		}
	} else if( get_post_type() == 'rb_portfolio' ){
		if( is_singular() ){
			$page_title = !empty(get_theme_mod('rb_portfolio_single_slug')) ? esc_html(get_theme_mod('rb_portfolio_single_slug')) : esc_html_x('Portfolio Single', 'frontend', 'ogo');
		} else {
			$page_title = !empty(get_theme_mod('rb_portfolio_slug')) ? esc_html(get_theme_mod('rb_portfolio_slug')) : esc_html_x('Portfolio', 'frontend', 'ogo');
		}
	} else if( is_archive() ){
		...
		if(is_author()){
			$page_title = sprintf( esc_html_x('Posted by %s', 'frontend', 'ogo'), get_the_author_meta('display_name'));
		}
	} else {
 ...
 * @return  mixed
 */
function ogo__option( $name ) {
	static $options;
	if (empty($options)) {
		global $wp_query;
		$pid = get_the_id();

		$theme_options = get_option(OGO__ID);
 ...
 * @return  strings
 */
function ogo__current_post_type() {
 ...
 * @since   1.0.0
 */
function ogo_logo( $logo_field = '', $logo_dimensions = '', $tag = 'h1' ){
 ...
 * @since   1.0.0
 */
function ogo__attributes( $attrs ) {
	$attributes = array();
