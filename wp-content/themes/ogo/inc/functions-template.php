 ...
 * @since   1.0.0
 */
function ogo__pagination( $query = null, $max_posts = '', $posts_per_page = '', $type = 'standart', $custom_max_num_pages = '' ) {
	global $wp_query;
	...
	if( !empty($links) ){
		$out .= '<nav class="paging-navigation '.$type.'">';
			$out .= '<div class="pagination loop-pagination">';
				$out .= $type != 'standart' ? '<div class="rb_button rb_load_more">'.esc_html_x( 'Load More', 'frontend', 'ogo' ).'</div>' : $links;
			$out .='</div>';
		$out .= '</nav>';
	}
 ...
 * @since   1.0.0
 */
function ogo_dimox_breadcrumbs(){
	/* === OPTIONS === */
	$text['home']	 = esc_html__( 'Home', 'ogo' ); // text for the 'Home' link
	$text['category'] = esc_html__( 'Archive by Category "%s"', 'ogo' ); // text for a category page
	$text['search']   = esc_html__( 'Search for "%s"', 'ogo' ); // text for a search results page
	$text['taxonomy'] = esc_html__( 'Archive by %s "%s"', 'ogo' );
	$text['tag']	  = esc_html__( 'Posts Tagged "%s"', 'ogo' ); // text for a tag page
	$text['author'] = esc_html__( 'Articles Posted by %s', 'ogo' ); // text for an author page
	$text['404']	  = esc_html__( 'Error 404', 'ogo' ); // text for the 404 page
	...
	if ( is_home() || is_front_page() || (empty( $post )) ) {
		...
		if ( is_category() ) {
		...
		} else if ( is_tax( array( 'rb_portfolio_tag', 'rb_portfolio_cat', 'rb_staff_member_department', 'rb_staff_member_position' ) ) ) {
			...
			$single_title = wp_kses( $single_title, array(
				"b"			=> array(),
				"em"		=> array(),
				"sup"		=> array(),
				"sub"		=> array(),
			...
			if ( $show_current ) { echo sprintf("%s", $before) . $single_title . $after; }
		...
		if ( get_query_var( 'paged' ) ) {
			echo sprintf("%s", $delimiter) . esc_html__( 'Page', 'ogo' ) . ' ' . get_query_var( 'paged' );
		}
 ...
 * @since   1.0.0
 */
function ogo__get_sidebar( $id = null, $custom_sidebar = '', $custom_sidebar_pos = '', $extra_class = '', $show_empty = false ){
	$page_type = 'page';
	$sidebar = null;
	$out = '';
	...
	if( !empty($check_sidebar) ){
		...
		$out = '<aside class="sidebar position_'.esc_attr($custom_sidebar_pos).' '.(!empty($extra_class) ? $extra_class : '').'">';
			$out .= '<p class="empty_custom_sb_title">'.esc_html_x('Fill the custom sidebar or choose anouther one.', 'frontend', 'ogo').'</p>';
		$out .= '</aside>';
	}
