...
add_filter( 'the_content_more_link', 'ogo__modify_read_more_link' );

function ogo__modify_read_more_link() {
	return ogo__read_more();
}

function ogo__post_featured( $post_hide_meta = '', $cropped = false, $layout = '' ){
...
function ogo__post_title( $post_hide_meta = '' ){
...
function ogo__post_category( $post_hide_meta = '' ){			
...
function ogo__post_author( $post_hide_meta = '' ){
...
function ogo__post_date( $post_hide_meta = '', $type = 'simple' ){
...
function ogo__post_comments( $post_hide_meta = '' ){
	$comments = get_comments_number();
	$out = '';

	if( $comments != '0' && strripos($post_hide_meta, 'comments') === false ){
		$out .= '<div class="coments_count">';
...
function ogo__post_meta( $post_hide_meta = '' ){
	if( strripos($post_hide_meta, 'author') === false || 
		strripos($post_hide_meta, 'date') === false || 
		( strripos($post_hide_meta, 'comments') === false ) && !empty(ogo__post_comments()) )
	{
		echo '<div class="post-meta-wrapper">';
			ogo__post_author( $post_hide_meta );
			ogo__post_date( $post_hide_meta );
			echo ogo__post_comments( $post_hide_meta );
		echo '</div>';
	}
}

function ogo__read_more( $title = '', $post_hide_meta = '', $type = 'simple' ){
	$post_permalink = get_permalink();
	$post_target    = '_self';
	$out 			= '';
	...
	$out .= '<div class="blog-readmore-wrap rb_button_wrapper '.esc_attr($type).'">';
		$out .= '<a class="blog-readmore rb_button" href="'. esc_url( $post_permalink ) .'" target="'. esc_attr( $post_target ) .'">';
			$out .= "<span>".esc_html($title)."</span>";
			$out .= $type == 'dashed' || $type == 'dashed_default' ? '<span class="dashes"></span>' : '';
		$out .= '</a>';
	$out .= '</div>';
...
function ogo__the_content( $max_length = '', $read_more = '', $post_hide_meta = '', $btn_style = 'simple' ){
