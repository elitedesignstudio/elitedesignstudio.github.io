...
function rb_vc_shortcode_sc_logo ( $atts = array(), $content = "" ){
	...
	$retina_h = !empty($image) ? (int)($image[2] / 2) : '';

	$width = empty($width) ? $image[1] : $width;
	$height = empty($height) ? $image[2] : $height;
	...
	$module_styles = $retina ? 'width:'.$retina_w.'px;' : 'width:'.(int)esc_attr($width).'px;';
	$module_styles .= $retina ? 'height:'.$retina_h.'px;' : ' height:'.(int)esc_attr($height).'px;';
	...
	$out .= "<div id='".$id."' class='site_logotype". ( !empty($el_class) ? " $el_class" : "" ) ."'>";
		$out .= "<a href='".esc_url(home_url('/'))."'>";
			if( !empty($image) ){
				...
				ob_start();
					ogo_logo('logotype', 'logo_dimensions', 'h3');
				$out .= ob_get_clean();
			}
		$out .= "</a>";
	$out .= "</div>";
